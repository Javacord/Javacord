...
public class ImplChannelCategory implements ChannelCategory {
	...
	/*
	 * This test case is special, as this will return true if at least one channel is accessible.
	 */
	@Override
	public boolean canBeAccessedBy(User user) {
		for (ServerChannel chan : this.getChannels()) {
			if(chan.canBeAccessedBy(user)) {
				return true;
			}
		}
		return false;
	...
	public boolean canBeFullyAccessedBy(User user) {
		return this.getChannels().stream().allMatch(chan-> {
			if(chan instanceof ChannelCategory) 
				return ((ChannelCategory)chan).canBeFullyAccessedBy(user);
			else
				return chan.canBeAccessedBy(user);
		});
	}
}
