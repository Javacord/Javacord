...
public interface ServerMessageChannel extends RegularServerChannel, TextChannel, Mentionable, Categorizable {
     ...
     */
    default ServerMessageChannelUpdater createUpdater() {
        return new ServerMessageChannelUpdater(this);
    }
    ...
    @Override
    default Optional<ServerMessageChannel> getCurrentCachedInstance() {
        return getApi().getServerById(getServer().getId()).flatMap(server ->
                server.getServerMessageChannelById(getId()));
    }

    @Override
    default CompletableFuture<ServerMessageChannel> getLatestInstance() {
        Optional<ServerMessageChannel> currentCachedInstance = getCurrentCachedInstance();
        if (currentCachedInstance.isPresent()) {
            return CompletableFuture.completedFuture(currentCachedInstance.get());
        } else {
            CompletableFuture<ServerMessageChannel> result = new CompletableFuture<>();
