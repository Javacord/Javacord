...
import org.javacord.event.message.MessageEvent;
 ...
 */
public abstract class ImplMessageEvent extends ImplEvent implements MessageEvent {
     ...
     */
    public ImplMessageEvent(Message message) {
     ...
     */
    public ImplMessageEvent(DiscordApi api, long messageId, TextChannel channel) {
        super(api);
    ...
    public CompletableFuture<Void> addReactionsToMessage(String... unicodeEmojis) {
        return addReactionsToMessage(Arrays.stream(unicodeEmojis)
                                             .map(ImplUnicodeEmoji::fromString)
                                             .toArray(Emoji[]::new));
    }
