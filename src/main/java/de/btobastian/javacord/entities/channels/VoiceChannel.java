...
public interface VoiceChannel extends Channel {
	...
	/**
	 * Checks if the given user can connect to the voice channel. In private chats
	 * (private channel or group channel) this always returns <code>true</code> if
	 * the user is part of the chat.
	 *
	 * @param user
	 *            The user to check.
	...
	default boolean canConnect(User user) {
		...
		return !severTextChannel.isPresent() || severTextChannel.get().hasAnyPermission(user,
				PermissionType.ADMINISTRATOR, PermissionType.VOICE_CONNECT);
	}

	/**
	 * Checks if the user of the connected account can connect to the voice channel.
	 * In private chats (private channel or group channel) this always returns
	 * {@code true} if the user is part of the chat.
	 *
	...
	/**
	 * Checks if the given user can mute other users in this voice channel. In
	 * private chats (private channel or group channel) this always returns
	 * <code>false</code>.
	 *
	 * @param user
	 *            The user to check.
	...
	default boolean canMuteUsers(User user) {
		...
		return !serverVoiceChannel.isPresent() || serverVoiceChannel.get().hasAnyPermission(user,
				PermissionType.ADMINISTRATOR, PermissionType.VOICE_MUTE_MEMBERS);
	}

	/**
	 * Checks if the user of the connected account can mute other users in this
	 * voice channel. In private chats (private channel or group channel) this
	 * always returns {@code false}.
	 *
	 * @return Whether the user of the connected account can mute other users or
	 *         not.
	 */
	 ...
	 * 
	 * @param user
	 *            The user to add to this voice-channel's connected-users list.
	 */
	 ...
	 * 
	 * @param user
	 *            The user to remove from this voice-channel's connected-users list.
	 */
