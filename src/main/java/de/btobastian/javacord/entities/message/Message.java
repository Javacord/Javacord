...
public interface Message extends DiscordEntity, Comparable<Message> {
	...
	static CompletableFuture<Void> delete(DiscordApi api, long channelId, long messageId) {
		return new RestRequest<Void>(api, HttpMethod.DELETE, RestEndpoint.MESSAGE_DELETE)
				.setUrlParameters(String.valueOf(channelId), String.valueOf(messageId)).setRatelimitRetries(25)
				.execute(res -> {
	...
	static CompletableFuture<Void> edit(DiscordApi api, long channelId, long messageId, String content,
		...
		return new RestRequest<Void>(api, HttpMethod.PATCH, RestEndpoint.MESSAGE)
				.setUrlParameters(String.valueOf(channelId), String.valueOf(messageId)).setBody(body)
				.execute(res -> null);
	}
	...
	static CompletableFuture<Void> addReaction(DiscordApi api, long channelId, long messageId, String unicodeEmoji) {
		return new RestRequest<Void>(api, HttpMethod.PUT, RestEndpoint.REACTION)
				...
				.setRatelimitRetries(50).execute(res -> null);
	}
	 ...
	 * @return A future to tell us if the action was successful.
	 */
	static CompletableFuture<Void> addReaction(DiscordApi api, String channelId, String messageId,
			String unicodeEmoji) {
		try {
	...
	static CompletableFuture<Void> removeAllReactions(DiscordApi api, long channelId, long messageId) {
		return new RestRequest<Void>(api, HttpMethod.DELETE, RestEndpoint.REACTION)
				.setUrlParameters(String.valueOf(channelId), String.valueOf(messageId)).execute(res -> null);
	}
	 ...
	 * with the actual name. The replacement happens as following:
	 * <ul>
	 * <li><b>User mentions</b>: <code>@nickname</code> if the user has a nickname,
	 * <code>@name</code> if the user has no nickname, <code>@invalid-user</code> if
	 * the user is not in the server.
	 * <li><b>Channel mentions</b>: <code>#name</code> if the text channel exists in
	 * the server, otherwise <code>#deleted-channel</code>
	 * <li><b>Custom emoji</b>: <code>:name:</code>. If the emoji is known, the real
	 * name is used, otherwise the name from the mention tag.
	 * </ul>
	 *
	...
	default String getReadableContent() {
		...
		while (channelMention.find()) {
			...
			String channelName = getChannel().asServerChannel().map(c -> c.getServer().getTextChannelById(channelId)
					.map(ServerChannel::getName).orElse("deleted-channel")).orElse("deleted-channel");
	...
	/**
	 * Gets the user author of the message. The author is not present, if it's a
	 * webhook for example.
	 *
	 ...
	 *
	 * @param cachedForever
	 *            Whether the message should be kept in cache forever or not.
	 */
	void setCachedForever(boolean cachedForever);

	/**
	 * Checks if the information of this message is still valid. If the message got
	 * removed from cache but you stored this objects somewhere it might have
	 * outdated content.
	 *
	...
	/**
	 * Whether this message is deleted or not. Deleted messages might still be in
	 * the cache for not more than 60 seconds.
	 *
	 ...
	 *
	 * @param unicodeEmoji
	 *            The unicode emoji of the reaction.
	 * @return The reaction for the given emoji.
	 */
	...
	/**
	 * Gets the server text channel of the message. Only present if the message was
	 * sent in a server.
	 *
	...
	/**
	 * Gets the private channel of the message. Only present if the message was sent
	...
	/**
	 * Gets the history of messages around this message. Half of the message will be
	 * older than the given message and half of the message will be newer. If there
	 * aren't enough older or newer messages, the actual amount of messages will be
	 * less than the given limit. It's also not guaranteed to be perfectly balanced.
	 *
	 * @param limit
	 *            The limit of messages to get.
	 * @return The history.
	...
	/**
	 * Checks if user can add reaction to this message
	 * 
	 * @param user
	 *            - The user object
	 * @param emote
	...
	default boolean canAddReaction(User user, Emoji emote) {
		...
		if (!STC.canAddNewRections(user)) {
			...
			return STC.getServer().getCustomEmojis().contains(emote);
		}
	}
