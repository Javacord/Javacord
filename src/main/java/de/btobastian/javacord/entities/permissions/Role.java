...
public interface Role extends DiscordEntity, Mentionable {
	...
	default Collection<PermissionType> getAllowedPermissions() {
		return Arrays.stream(PermissionType.values())
				.filter(type -> getPermissions().getState(type) == PermissionState.ALLOWED).collect(Collectors.toSet());
	}
	...
	default Collection<PermissionType> getUnsetPermissions() {
		return Arrays.stream(PermissionType.values())
				.filter(type -> getPermissions().getState(type) == PermissionState.NONE).collect(Collectors.toSet());
	}
	...
	default CompletableFuture<String> setPermissions(Permissions perms) {
		JSONObject body = new JSONObject();
		body.put("permissions", ((ImplPermissions) perms).getAllowed());
		return new RestRequest<String>(this.getApi(), HttpMethod.PATCH, RestEndpoint.ROLE)
				.setUrlParameters(String.valueOf(this.getServer().getId()), String.valueOf(this.getId())).setBody(body)
				.execute(res -> res.getStatusText());
	}
}
